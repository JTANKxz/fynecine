...
class ShortController extends Controller
{
    private function profile(Request $request): Profile
    {
        ...
        abort_unless($id, 400, 'Header Profile-Id é obrigatório.');
        return $request->user()->profiles()->findOrFail($id);
    }
    ...
    public function feed(Request $request): JsonResponse
    {
        ...
        foreach ($shorts as $short) {
            ShortInteraction::updateOrCreate(
                ['short_id' => $short->id, 'user_id' => $request->user()->id, 'profile_id' => $profile->id, 'type' => 'impression'],
    ...
    private function payload(Short $short, Profile $profile): array
    {
        return [
            ...
            'related_content' => $short->related_type ? [
                'type' => $short->related_type, 'id' => $short->related_id, 'title' => $short->related_title,
            ] : null,
            'is_liked' => $short->interactions()->where('profile_id', $profile->id)->where('type', 'like')->exists(),
        ];
    }
}
