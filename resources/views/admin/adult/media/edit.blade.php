...
@section('title', 'Editar Mídia Adulta')
...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold">Editar Mídia</h2>
</div>

<form action="{{ route('admin.adult.media.update', $media->id) }}" method="POST" class="max-w-2xl bg-neutral-900 p-6 rounded-lg border border-neutral-800">
    @csrf
    @method('PUT')
    ...
    <div class="mb-4">
        ...
        <select name="adult_gallery_id" id="adult_gallery_id" class="w-full bg-neutral-800 border border-neutral-700 rounded px-3 py-2 text-white focus:outline-none focus:border-netflix">
            ...
            @foreach($galleries as $gallery)
                <option value="{{ $gallery->id }}" {{ $media->adult_gallery_id == $gallery->id ? 'selected' : '' }}>{{ $gallery->title }}</option>
            @endforeach
        </select>
    </div>
        ...
        <div>
            ...
            <select name="adult_model_id" id="adult_model_id" class="w-full bg-neutral-800 border border-neutral-700 rounded px-3 py-2 text-white focus:outline-none focus:border-netflix">
                ...
                @foreach($models as $model)
                    <option value="{{ $model->id }}" {{ $media->adult_model_id == $model->id ? 'selected' : '' }}>{{ $model->name }}</option>
                @endforeach
            </select>
        </div>
        <div>
            ...
            <select name="adult_category_id" id="adult_category_id" class="w-full bg-neutral-800 border border-neutral-700 rounded px-3 py-2 text-white focus:outline-none focus:border-netflix">
                ...
                @foreach($categories as $category)
                    <option value="{{ $category->id }}" {{ $media->adult_category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
            ...
            <select name="type" id="type" class="w-full bg-neutral-800 border border-neutral-700 rounded px-3 py-2 text-white focus:outline-none focus:border-netflix">
                <option value="image" {{ $media->type == 'image' ? 'selected' : '' }}>Foto/Imagem</option>
    ...
    <div class="grid grid-cols-2 gap-4 mb-6">
        <div>
            ...
            <input type="number" name="order" id="order" value="{{ $media->order }}" class="w-full bg-neutral-800 border border-neutral-700 rounded px-3 py-2 text-white focus:outline-none focus:border-netflix">
        </div>
        <div class="flex items-end pb-2">
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" class="sr-only peer" {{ $media->is_active ? 'checked' : '' }}>
