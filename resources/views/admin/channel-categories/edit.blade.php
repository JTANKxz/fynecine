...
@section('content')
<section>
    <h2 class="text-xl font-bold mb-4">Editar Categoria: {{ $category->name }}</h2>
    ...
    <form action="{{ route('admin.channel-categories.update', $category->id) }}" method="POST" class="bg-neutral-900 p-5 rounded space-y-4">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="channels[]" multiple
                    ...
                    @foreach($channels as $ch)
                        <option value="{{ $ch->id }}" {{ in_array($ch->id, old('channels', $category->channels->pluck('id')->toArray())) ? 'selected' : '' }}>
                            {{ $ch->name }}
                        </option>
                    @endforeach
                </select>
