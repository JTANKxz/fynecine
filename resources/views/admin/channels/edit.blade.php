...
@section('content')
<section>
    <h2 class="text-xl font-bold mb-4">Editar Canal: {{ $channel->name }}</h2>
    ...
    <form action="{{ route('admin.channels.update', $channel->id) }}" method="POST" class="bg-neutral-900 p-5 rounded space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="image_url" value="{{ old('image_url', $channel->image_url) }}"
                    class="w-full p-2 bg-neutral-800 rounded focus:ring-2 focus:ring-netflix outline-none"
        ...
        <div>
            ...
            <select name="categories[]" multiple
                ...
                @foreach($categories as $cat)
                    <option value="{{ $cat->id }}" {{ in_array($cat->id, old('categories', $channel->categories->pluck('id')->toArray())) ? 'selected' : '' }}>
                        {{ $cat->name }}
                    </option>
                @endforeach
            </select>
        ...
        @if($channel->image_url)
            <div>
                <label class="block text-sm text-neutral-400 mb-1">Preview da Logo</label>
                <img src="{{ $channel->image_url }}" class="w-20 h-20 object-contain rounded bg-neutral-800 p-2">
            </div>
        @endif
