...
<section>
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-xl font-bold">Novo Cupom VIP</h2>
    ...
    <form action="{{ route('admin.coupons.store') }}" method="POST" class="bg-neutral-900 p-6 rounded-xl border border-neutral-800 space-y-6">
        @csrf
        ...
        <div class="grid md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <p class="mt-1 text-xs text-neutral-500">Ex: VIP30, LANCAMENTO</p>
            </div>
            ...
            <div>
                ...
                <input type="number" name="max_uses" 
                    class="w-full p-3 bg-neutral-800 border border-neutral-700 rounded-lg focus:ring-2 focus:ring-netflix focus:border-netflix outline-none transition" 
                    value="{{ old('max_uses') }}" min="1" placeholder="Ilimitado">
                <p class="mt-1 text-xs text-neutral-500">Deixe vazio para uso ilimitado (1 por pessoa).</p>
            </div>
        </div>

        {{-- Vincular a um Plano VIP --}}
        <div class="p-4 bg-netflix/5 border border-netflix/20 rounded-lg">
            <label class="block text-sm font-bold text-netflix mb-2">Vincular a um Plano VIP Predefinido (Recomendado)</label>
        ...
        <div class="grid md:grid-cols-2 gap-6 pt-4 border-t border-neutral-800">
            ...
            <div>
                ...
                <select name="plan" class="w-full p-3 bg-neutral-800 border border-neutral-700 rounded-lg focus:ring-2 focus:ring-netflix outline-none transition">
                    <option value="basic" {{ old('plan') == 'basic' ? 'selected' : '' }}>Basic (Até 2 Perfis)</option>
                    <option value="premium" {{ old('plan', 'premium') == 'premium' ? 'selected' : '' }}>Premium (Até 5 Perfis)</option>
                </select>
            </div>
