...
<section class="max-w-2xl">
    ...
    <div class="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 md:p-8 shadow-2xl">
        <form action="{{ route('admin.events.links.store', $event->id) }}" method="POST" class="space-y-6">
            ...
            {{-- Bunny fields --}}
            <div id="bunny_fields" class="hidden grid md:grid-cols-2 gap-6 border-t border-neutral-800 pt-6">
                <div class="space-y-2">
                    <label class="block text-sm font-bold text-neutral-400 uppercase tracking-widest text-[10px]">Link Path (opcional)</label>
                    <input type="text" name="link_path" class="w-full bg-black border border-neutral-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-netflix transition" placeholder="/video-folder/">
                </div>
                <div class="space-y-2">
                    <label class="block text-sm font-bold text-neutral-400 uppercase tracking-widest text-[10px]">Expiração (horas)</label>
                    <input type="number" name="expiration_hours" value="4" class="w-full bg-black border border-neutral-800 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-netflix transition">
                </div>
            </div>
            ...
            <div class="border-t border-neutral-800 pt-6">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div class="space-y-1">
                        ...
                        <input type="text" name="user_agent" class="w-full bg-black border border-neutral-800 rounded-xl px-4 py-2 text-white text-sm focus:border-netflix outline-none" placeholder="ExoPlayer/2.18.1">
                    </div>
                    <div class="space-y-1">
                        <label class="block text-[10px] font-bold text-neutral-500 uppercase">Referer</label>
                        <input type="text" name="referer" class="w-full bg-black border border-neutral-800 rounded-xl px-4 py-2 text-white text-sm focus:border-netflix outline-none" placeholder="https://site.com">
                    </div>
                    <div class="space-y-1">
                        <label class="block text-[10px] font-bold text-neutral-500 uppercase">Origin</label>
                        <input type="text" name="origin" class="w-full bg-black border border-neutral-800 rounded-xl px-4 py-2 text-white text-sm focus:border-netflix outline-none" placeholder="https://site.com">
                    </div>
                    <div class="space-y-1">
                        <label class="block text-[10px] font-bold text-neutral-500 uppercase">Cookies</label>
                        <input type="text" name="cookie" class="w-full bg-black border border-neutral-800 rounded-xl px-4 py-2 text-white text-sm focus:border-netflix outline-none" placeholder="key=value;">
                    </div>
                </div>
            </div>
